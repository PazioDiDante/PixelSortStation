...
namespace Nika_Vizard_Station.Services
{
    ...
    public class GifWriter : IDisposable
    {
        ...
        public void WriteFrame(BitmapSource bitmapSource, int Delay = 0)
        {
            lock (_syncLock)
                using (var gifStream = new MemoryStream())
                {
                    // Преобразуйте BitmapSource в Bitmap
                    BitmapEncoder encoder = new PngBitmapEncoder();
                    encoder.Frames.Add(BitmapFrame.Create(bitmapSource));
                    encoder.Save(gifStream);

                    // Строка глобальной таблицы цветов
                    if (_firstFrame)
                        InitHeader(gifStream, _writer, bitmapSource.PixelWidth, bitmapSource.PixelHeight);

                    // Запись блока управления графикой
                    WriteGraphicControlBlock(gifStream, _writer, Delay == 0 ? DefaultFrameDelay : Delay);

                    // Запись блока изображения
                    WriteImageBlock(gifStream, _writer, !_firstFrame, 0, 0, bitmapSource.PixelWidth, bitmapSource.PixelHeight);
                }
