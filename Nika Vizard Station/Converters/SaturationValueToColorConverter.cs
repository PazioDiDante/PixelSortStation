...
{
    public class SaturationValueToColorConverter : IMultiValueConverter
    {
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            if (values.Length == 3 && values[0] is double sliderValue && values[1] is double min && values[2] is double max)
            {
                ...
                Color startColor = (Color)ColorConverter.ConvertFromString("#FF9C9C9C"); 
                Color endColor = (Color)ColorConverter.ConvertFromString("#FFDF815D"); 
