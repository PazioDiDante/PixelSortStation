...
{
    public class FadeValueToColorConverter : IMultiValueConverter
    {
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            if (values.Length == 3 && values[0] is double sliderValue && values[1] is double min && values[2] is double max)
            {
                ...
                Color startColor = (Color)ColorConverter.ConvertFromString("#FF121212"); 
                Color endColor = (Color)ColorConverter.ConvertFromString("#FFC1A74A"); 
